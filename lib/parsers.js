...
function Parser(text) {
    ...
    function parseBinaryExpression(level) {
        ...
        var oper = tryParseBinaryOperator(level);
        
        if (!oper)
            return expr;
        
        if (oper.value === '+')
            return expressions.add(expr, parseBinaryExpression(level + 1));

        if (oper.value === '-')
            return expressions.subtract(expr, parseBinaryExpression(level + 1));

        if (oper.value === '*')
            return expressions.multiply(expr, parseBinaryExpression(level + 1));

        if (oper.value === '/')
            return expressions.divide(expr, parseBinaryExpression(level + 1));

        if (oper.value === '=')
            return expressions.match(expr, parseBinaryExpression(level + 1));
    }
